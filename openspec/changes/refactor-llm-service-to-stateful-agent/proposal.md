...
- Replace the Flask application with an asynchronous FastAPI service while preserving the existing one-shot LLM endpoints during migration.
...
- `fastapi-llm-runtime`: Covers the FastAPI service contract, legacy endpoint compatibility, asynchronous model access, health reporting, and streaming-ready responses.
...
- Adds Python, Java, and frontend tests for multi-turn state, scope isolation, tool behavior, context compaction, compatibility endpoints, and degraded operation.
