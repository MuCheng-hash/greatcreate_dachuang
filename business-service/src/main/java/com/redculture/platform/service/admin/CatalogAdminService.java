...
public class CatalogAdminService {
    ...
    public PageResult<CatalogEntityVO> page(EntityType entityType, String keyword, Long pageNum, Long pageSize) {
    ...
    public CatalogRelationVO createRelation(CatalogRelationRequest request) {
        ...
        if (request.getSourceType() == EntityType.STORY && request.getTargetType() != EntityType.ACTIVITY_PLAN) {
    ...
    public CatalogRelationVO relation(String kind, Long relationId) {
        ...
        return switch (kind) {
            case "site_event" -> { SiteEventRel item=siteEventRelMapper.selectById(relationId); yield item==null?null:relationVO(EntityType.SITE,item.getSiteId(),EntityType.EVENT,item.getEventId(),enumValue(item.getRelationType()),item.getRemark(),kind,relationId); }
            case "site_hero" -> { SiteHeroRel item=siteHeroRelMapper.selectById(relationId); yield item==null?null:relationVO(EntityType.SITE,item.getSiteId(),EntityType.HERO,item.getHeroId(),enumValue(item.getRelationType()),item.getRemark(),kind,relationId); }
            case "event_hero" -> { EventHeroRel item=eventHeroRelMapper.selectById(relationId); yield item==null?null:relationVO(EntityType.EVENT,item.getEventId(),EntityType.HERO,item.getHeroId(),enumValue(item.getRelationType()),item.getContributionText(),kind,relationId); }
            case "memorial_site" -> { MemorialSiteRel item=memorialSiteRelMapper.selectById(relationId); yield item==null?null:relationVO(EntityType.MEMORIAL,item.getMemorialId(),EntityType.SITE,item.getSiteId(),enumValue(item.getRelationType()),null,kind,relationId); }
            case "memorial_hero" -> { MemorialHeroRel item=memorialHeroRelMapper.selectById(relationId); yield item==null?null:relationVO(EntityType.MEMORIAL,item.getMemorialId(),EntityType.HERO,item.getHeroId(),enumValue(item.getRelationType()),null,kind,relationId); }
            case "memorial_event" -> { MemorialEventRel item=memorialEventRelMapper.selectById(relationId); yield item==null?null:relationVO(EntityType.MEMORIAL,item.getMemorialId(),EntityType.EVENT,item.getEventId(),enumValue(item.getRelationType()),null,kind,relationId); }
            case "story_entity" -> { StoryEntityRel item=storyEntityRelMapper.selectById(relationId); yield item==null?null:relationVO(EntityType.STORY,item.getStoryId(),item.getEntityType(),item.getEntityId(),enumValue(item.getRelationType()),null,kind,relationId); }
            default -> null;
        };
    }
    ...
    private void apply(EntityType type, Object entity, CatalogEntityRequest request) {
        switch (type) {
            case RESOURCE -> { LocalEduResource item=(LocalEduResource) entity; item.setResourceCode(clean(request.getCode())); item.setResourceName(clean(request.getName())); item.setResourceAlias(clean(request.getAlias())); item.setRegionId(request.getRegionId()); item.setAddress(clean(request.getAddress())); item.setLongitude(request.getLongitude()); item.setLatitude(request.getLatitude()); item.setIntro(clean(request.getSummary())); item.setEducationValue(clean(request.getDetail())); item.setTargetGrade(clean(request.getTargetGrade())); if (item.getResourceCategory()==null) item.setResourceCategory(ResourceCategory.OTHER); }
    ...
    private CatalogEntityVO toVO(EntityType type, Object entity) {
        ...
        if(entity instanceof LocalEduResource x){vo.setEntityId(x.getResourceId());vo.setCode(x.getResourceCode());vo.setName(x.getResourceName());vo.setAlias(x.getResourceAlias());vo.setRegionId(x.getRegionId());vo.setAddress(x.getAddress());vo.setLongitude(x.getLongitude());vo.setLatitude(x.getLatitude());vo.setSummary(x.getIntro());vo.setDetail(x.getEducationValue());vo.setTargetGrade(x.getTargetGrade());vo.setReviewStatus(enumValue(x.getReviewStatus()));vo.setActive(x.getActive());vo.setCreatedAt(x.getCreatedAt());vo.setUpdatedAt(x.getUpdatedAt());}
