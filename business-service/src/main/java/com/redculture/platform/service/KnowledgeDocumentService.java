...
public class KnowledgeDocumentService {
    ...
    public void retry(Long id, AuthCurrentUserVO user) { KnowledgeDocument document = detail(id, user); KnowledgeIngestJob job = jobs.selectOne(new LambdaQueryWrapper<KnowledgeIngestJob>().eq(KnowledgeIngestJob::getDocumentId, id)); if (!"FAILED".equals(document.getStatus()) || job == null) throw new IllegalStateException("only failed documents can be retried"); document.setStatus("PENDING"); documents.updateById(document); job.setStatus("PENDING"); job.setCurrentNode("VALIDATE"); job.setErrorSummary(null); job.setFinishedAt(null); jobs.updateById(job); enqueue(job.getId()); }
