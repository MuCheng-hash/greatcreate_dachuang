...
const elements = {
    ...
    planGradeInput: document.querySelector("#planGradeInput"),
    ...
    planPracticeInput: document.querySelector("#planPracticeInput"),
    generatePlanButton: document.querySelector("#generatePlanButton"),
    savePlanDraftButton: document.querySelector("#savePlanDraftButton"),
    planResult: document.querySelector("#planResult"),
    planCitations: document.querySelector("#planCitations"),
...
function bindEvents() {
    ...
    elements.generatePlanButton?.addEventListener("click", () => {
        void generateTeachingPlan();
    });

    elements.savePlanDraftButton?.addEventListener("click", () => {
        void saveGeneratedPlanDraft();
    });
...
async function generateTeachingPlan() {
    if (!state.currentSchoolDetail?.school?.schoolId) {
        if (elements.planStatus) {
            elements.planStatus.textContent = "请先选择学校";
        }
        return;
    }

    const payload = buildTeachingPlanPayload();
    if (!payload.grade || !payload.theme) {
        elements.planStatus.textContent = "请补充年级和主题";
        return;
    }

    elements.planStatus.textContent = "生成中";
    if (elements.generatePlanButton) {
        elements.generatePlanButton.disabled = true;
    }
    if (elements.savePlanDraftButton) {
        elements.savePlanDraftButton.disabled = true;
    }

    try {
        const response = await requestJson("/api/ai/teaching-plans/generate", {
            method: "POST",
            body: JSON.stringify(payload)
        });
        ...
        if (elements.savePlanDraftButton) {
            elements.savePlanDraftButton.disabled = false;
        }
    } catch (error) {
        elements.planStatus.textContent = "生成失败";
        if (elements.planResult) {
            elements.planResult.innerHTML = "<p>教学方案生成失败，请稍后重试或调整输入。</p>";
        }
    } finally {
        if (elements.generatePlanButton) {
...
async function saveGeneratedPlanDraft() {
    const plan = state.generatedTeachingPlan;
    if (!plan || !state.currentSchoolDetail?.school?.schoolId) {
        return;
    }

    elements.planStatus.textContent = "保存中";
    try {
        ...
        const payload = {
            ...
            theme: plan.theme,
            activityType: plan.activityType || elements.planActivityTypeInput?.value || "classroom",
            grade: plan.grade,
            durationMinutes: plan.durationMinutes,
            objectives: plan.objectives || [],
            activityFlow: plan.activityFlow || [],
            preparation: plan.preparation || [],
            safetyNotes: plan.safetyNotes || [],
            reflection: plan.reflection || [],
            evaluation: plan.evaluation || []
        };
        await requestJson("/api/ai/teaching-plans/save-draft", {
            method: "POST",
            body: JSON.stringify(payload)
        });
        elements.planStatus.textContent = "已保存草稿";
    } catch (error) {
        elements.planStatus.textContent = "保存失败";
    }
}
...
function buildTeachingPlanPayload() {
    const payload = {
        ...
        activityType: elements.planActivityTypeInput?.value || "classroom",
...
function renderTeachingPlanResponse(plan) {
    ...
    const sections = [
        ["教学目标", plan.objectives],
        ["资源依据", plan.resourceBasis],
        ["活动流程", plan.activityFlow],
        ["课前准备", plan.preparation],
        ["现场任务", plan.fieldTasks],
        ["安全提示", plan.safetyNotes],
        ["课后反思", plan.reflection],
        ["评价方式", plan.evaluation]
    ];
    elements.planResult.innerHTML = `
        <article class="generated-plan">
            <h3>${escapeHtml(plan.theme || "生成教学方案")}</h3>
            <div class="resource-meta">
                ${plan.grade ? `<span>${escapeHtml(plan.grade)}</span>` : ""}
                ...
                ${plan.practiceRequired ? "<span>实践活动</span>" : ""}
            </div>
            ${plan.message ? `<p>${escapeHtml(plan.message)}</p>` : ""}
            ${sections.map(([title, items]) => renderGeneratedPlanSection(title, items)).join("")}
            ${(plan.followUpSuggestions || []).length ? renderGeneratedPlanSection("后续追问", plan.followUpSuggestions) : ""}
        </article>
    `;
    renderPlanCitations(plan.citations || []);
}

function renderGeneratedPlanSection(title, items) {
    const cleanItems = Array.isArray(items) ? items.filter(Boolean) : [];
    if (!cleanItems.length) {
        return "";
    }
    return `
        <section class="generated-plan__section">
...
function renderPlanCitations(citations) {
    if (!elements.planCitations) {
        return;
    }
    elements.planCitations.innerHTML = Array.isArray(citations) && citations.length
        ? citations.map((citation) => `
            <article>
                <p><strong>${escapeHtml(citation.title || citation.citationId || "引用来源")}</strong></p>
                ${citation.excerpt ? `<p>${escapeHtml(citation.excerpt)}</p>` : ""}
                <div class="resource-meta">
                    ${citation.sourceType ? `<span>${escapeHtml(citation.sourceType)}</span>` : ""}
                    ${citation.relatedEntityType ? `<span>${escapeHtml(citation.relatedEntityType)}</span>` : ""}
                </div>
            </article>
        `).join("")
        : "";
...
function syncPlanFormWithSchool(detail) {
    const themes = buildSchoolThemeSummaries(detail.resources || []);
    if (elements.planThemeInput && themes[0]?.title && !elements.planThemeInput.value.trim()) {
        elements.planThemeInput.value = themes[0].title;
    }
    state.generatedTeachingPlan = null;
    if (elements.savePlanDraftButton) {
        elements.savePlanDraftButton.disabled = true;
    }
    if (elements.planStatus) {
        elements.planStatus.textContent = "待命";
    }
    if (elements.planResult) {
        elements.planResult.innerHTML = "<p>生成后的教学方案会显示在这里。</p>";
    }
    if (elements.planCitations) {
